...
import './index.css';
import '../AssistantsList/index.css';
import {BACKEND_URL, BASE_PATH} from "../../Config";
import {AuthContext} from "../../auth";


function Chat({assistantId}) {
    ...
    const getChatSession = async (assistantId) => {
        try {
            const url = `${BACKEND_URL}/api/sessions/current?assistantId=${assistantId}`;
    ...
    return <>
        {error && <h1>{error}</h1>}
        {!error && sessionId && (
            <div className="container test-chat">
